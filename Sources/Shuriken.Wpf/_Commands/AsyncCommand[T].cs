...
namespace Shuriken
{
    ...
    public sealed class AsyncCommand<T> : ParameterizedCommand<T>
    {
        ...
        /// <param name="options">The options.</param>
        /// <exception cref="ArgumentNullException"><paramref name="execute" /> is <c>null</c>.</exception>
        public AsyncCommand([NotNull] Func<T, CancellationToken, Task> execute, Func<T, bool> canExecute = null, CommandOptions options = null)
            : base(true, canExecute, options ?? CommandOptions.DefaultAsync)
        {
            if (execute == null)
            {
        ...
        /// <exception cref="ArgumentNullException"><paramref name="execute" /> is <c>null</c>.</exception>
        public AsyncCommand(
            [NotNull] Func<T, CommandExecutionController, CancellationToken, Task> execute,
